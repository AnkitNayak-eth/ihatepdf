...
export const LaserFlow: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    function initThree() {
      ...
      const setSizeNow = () => {
        ...
        const w =
          mount.offsetWidth || mount.clientWidth || (mount.getBoundingClientRect && mount.getBoundingClientRect().width) || 1;
        const h =
          mount.offsetHeight || mount.clientHeight || (mount.getBoundingClientRect && mount.getBoundingClientRect().height) || 1;
        ...
        if (!sizeChanged && !dprChanged && w > 1) return;
      ...
      const adjustDprIfNeeded = (now: number) => {
        ...
        if (avgFps < lowerThresh) {
          next = Math.max(dprFloor, Math.min(base, currentDprRef.current * 0.85));
        } else if (avgFps > upperThresh && currentDprRef.current < base) {
          next = Math.max(dprFloor, Math.min(base, currentDprRef.current * 1.1));
        }
      ...
      const animate = () => {
        ...
        if (!hasReachedFadeOne) {
          const fadeDur = 0.5;
          fade = Math.min(1, fade + cdt / fadeDur);
          uniforms.uFade.value = fade;
          if (fade >= 1) hasReachedFadeOne = true;
        }

        if (Math.floor(t * 60) % 60 === 0 && lastSizeRef.current.width <= 1) {
          setSizeNow();
        }
    ...
    return () => {
      ...
      const canvas = canvasRef.current;
      if (canvas && (canvas as any).cleanupPointerEvents) {
  ...
  return (
    <div ref={mountRef} className={`w-full h-full relative overflow-hidden ${className || ''}`} style={style}>
